...
func _process(delta):
	...
	if Input.is_action_just_pressed("fireColdProjectile"):
		go.spawn_instance("ColdProjectile", self.position.x, self.position.y)
		if get_parent().buddy == true:
			go.spawn_instance("ColdProjectile", self.position.x + 80, self.position.y + 20) 
	pass
